...
- [ ] P24 Rings visualizing Near and Far properties only appear at 0 elevation - should be aligned with elevation of stems (so they're visible where these attributes are being adjusted).
- [ ] P25 Adding stem should always locate stem where camera is looking - including elevation. Currently it can place the stem on a different surface e.g. if there is a path "above" the camera at a higher elevation.
